...
DEVELOPER_LOG_DIRS = (LOG_DIR, SEARCH_LOG_DIR, PRODUCT_LOG_DIR, DOWNLOAD_LOG_DIR, COMPOSE_LOG_DIR, TIMING_LOG_DIR)
...
def load_settings():
    debug_log("load_settings from %s", SETTINGS_PATH)
    if SETTINGS_PATH.exists():
        try:
            return json.loads(SETTINGS_PATH.read_text(encoding="utf-8"))
        except Exception:
            log_exception("Could not read settings file")
            return {}
    return {}


def save_settings(settings):
    debug_log("save_settings to %s", SETTINGS_PATH)
