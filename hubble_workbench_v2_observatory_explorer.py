...
class HubbleWorkbench(DebugConsoleMixin, DeveloperToolsMixin, BetterSourcesMixin, ProductBrowserMixin, SearchWorkflowMixin, HlaWorkflowMixin, AppUtilitiesMixin, QualitySettingsMixin, TargetGalleryMixin, DependencyStatusMixin, BrowserActivityMixin, ObservatoryWorkflowMixin, ComposeWorkflowMixin, ProjectWorkflowMixin, PreviewWorkflowMixin, DownloadWorkflowMixin, ProductScoringMixin, MastSearchHelperMixin, tk.Tk):
    ...
    def build_browser_tab(self):
        ...
        self.hla_button = ttk.Button(top_action_row, text="Search HLA Fallback", command=self.hla_search_async)
        self.hla_button.pack(side="left", padx=(8, 0))
        self.products_button = ttk.Button(top_action_row, text="Get Products", command=self.products_async)
        self.products_button.pack(side="left", padx=(8, 0))
        self.all_products_button = ttk.Button(top_action_row, text="Get All Products", command=self.products_all_async)
        self.all_products_button.pack(side="left", padx=(8, 0))
        self.download_button = ttk.Button(top_action_row, text="Download Selected Products", command=self.download_selected_async)
        self.download_button.pack(side="left", padx=(8, 0))
        self.stop_browser_button = ttk.Button(top_action_row, text="Stop", command=self.cancel_browser_activity, state="disabled")
